...
<div id="chatWidget" class="fixed bottom-6 right-6 z-50">
    <!-- Chat Toggle Button -->
    <button id="chatToggle" class="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-full p-4 shadow-lg transition-all transform hover:scale-110 flex items-center justify-center">
        ...
        <span id="chatBadge" class="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center hidden">1</span>
    </button>

    <!-- Chat Window -->
    <div id="chatWindow" class="hidden absolute bottom-16 right-0 w-80 h-96 bg-white rounded-lg shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-blue-500 to-purple-600 text-white p-4 flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h3 class="font-semibold">BlueLife Support</h3>
        ...
        <div class="flex-1 flex flex-col">
            ...
            <div id="quickActions" class="p-4 border-b border-gray-200">
                ...
                <div class="space-y-2">
                    <button onclick="selectQuickAction('appointment')" class="w-full text-left p-2 hover:bg-gray-100 rounded-lg text-sm transition-colors">
                        📅 Book an Appointment
                    </button>
                    <button onclick="selectQuickAction('emergency')" class="w-full text-left p-2 hover:bg-gray-100 rounded-lg text-sm transition-colors">
                        🚨 Emergency Information
                    </button>
                    <button onclick="selectQuickAction('billing')" class="w-full text-left p-2 hover:bg-gray-100 rounded-lg text-sm transition-colors">
                        💳 Billing Questions
                    </button>
                    <button onclick="selectQuickAction('insurance')" class="w-full text-left p-2 hover:bg-gray-100 rounded-lg text-sm transition-colors">
                        🏥 Insurance Coverage
                    </button>
                    <button onclick="selectQuickAction('general')" class="w-full text-left p-2 hover:bg-gray-100 rounded-lg text-sm transition-colors">
                        💬 General Questions
                    </button>
                </div>
            </div>
        ...
        <div id="chatInput" class="p-4 border-t border-gray-200" style="display: none;">
            <div class="flex space-x-2">
                <input type="text" id="messageInput" placeholder="Type your message..." class="flex-1 p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" onkeypress="handleKeyPress(event)">
                <button onclick="sendMessage()" class="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-lg transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
class ChatWidget {
    ...
    getWelcomeMessage(context) {
        const messages = {
            ...
            'general': "Hello! I'm here to help with any questions about BlueLife Hospital. What would you like to know?"
        };
        
        return messages[context] || "Hello! I'm here to help you with any questions about BlueLife Hospital. How can I assist you today?";
    }

    getContextSuggestions(context) {
        const suggestions = {
            'appointment': "You can also book appointments online at bluelifehospital.com/appointment or call (555) 123-APPT.",
    ...
    async sendMessage(message) {
        ...
        setTimeout(() => {
            ...
            this.addMessage('bot', response, new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}));
    ...
    generateResponse(message) {
        ...
        // Insurance queries
        if (lowerMessage.includes('insurance') || lowerMessage.includes('coverage') || lowerMessage.includes('cost')) {
        ...
        if (lowerMessage.includes('location') || lowerMessage.includes('address') || lowerMessage.includes('directions')) {
            return "BlueLife Hospital is located at 123 Healthcare Drive, Medical City. We're easily accessible by car with free parking, and public transportation stops right outside. Would you like detailed directions?";
        }
